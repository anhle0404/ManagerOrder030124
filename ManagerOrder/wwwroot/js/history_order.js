        ...
        success: function (result) {
            ...
            if (parseInt(result) != 0) {
                ...
                $.each(result, function (key, item) {
                    ...
                    html += `<tr tabindex="0">
                            <td class="text-nowrap text-center"><input class="form-check-input" type="checkbox" name="IsApproved" value="${item.Id}" onclick="return onSelected(event);"/></td>
                    ...
                    htmlCard += `<div class="card mb-3">
                                    <div class="card-hearder border-bottom p-2 d-flex flex-nowrap align-items-start ${bgStatus}">
                                        <input class="form-check-input me-2" type="checkbox" name="IsApproved" value="${item.Id}" onclick="return onSelected(event);"/>
...
function onDelivery(deliveryStatus) {
    ...
    if (listSelected.length <= 0) {
        ...
        $.ajax({
            ...
            contentType: 'application/json',
            success: function (result) {
                const orderIsApproved = result.filter(v => v.order.IsApproved === 1);
                const orderNotApproved = result.filter(v => v.order.IsApproved === 0);
                if (orderNotApproved.length == 0) {
                    alert('Tất cả đơn hàng đã được duyệt!')
                } else if (orderIsApproved.length > 0) {
                    var ans = confirm('Những đơn hàng đã duyệt sẽ được bỏ qua.\nBạn có muốn tiếp tục?');
                    if (ans) {
                        var html = '';
                        $.each(orderNotApproved, (key, item) => {
                            var htmlDetail = '';
